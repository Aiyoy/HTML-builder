            ...
            try {
                ...
                for (const file of files) {
                    if (!file.isDirectory()) {
                        ...
                        createCopyFolder(path.join(pathPrew, file.name), path.join(pathNew, file.name));
                    }
                }        
            } catch (err) {
                console.error(err);
...
async function addTags(data) {
    ...
    for(let tag of tags) {
      ...
      const sample = await fs.promises.readFile(path.join(componentsPath, `${tagName}.html`));
